...
class __vec_iterator {
 public:
  ...
  template <typename _U,
            typename = std::enable_if<std::is_same<const _U, _T>::value>>
...
class vector_2 {
 public:
  using alloc_type =
      typename std::allocator_traits<_Allocator>::template rebind_alloc<_T>;
 ...
 public:
  vector_2();
  vector_2(size_type);
  vector_2(std::initializer_list<value_type> const&);
  vector_2(const vector_2&);
  vector_2(vector_2&&);
  ~vector_2();
  ...
  void swap(vector_2& other);
  void pop_back();

 private:
  void __destroy();
  void __move(vector_2&&);

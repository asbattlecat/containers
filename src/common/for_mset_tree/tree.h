...
class __tree_iterator {
 public:
  ...
  struct __node {
    ...
    __node(const_reference __ref, __node *__parent)
        : parent_(__parent), value_(__ref) {}

    __node(const __node &__n)
        : left_(__n.left_),
          right_(__n.right_),
          parent_(__n.parent_),
          value_(__n.value_) {}

    inline bool operator==(const __node &__n) const {
      return this->left_ == __n.right_ && this->right_ == __n.left_ &&
    ...
    inline bool _is_left_child() const {
      return this->_has_parent() && this->parent_->left_ == this;
    }

    inline bool _is_right_child() const {
...
namespace s21 {
template <typename _T, class _Compare = std::less<_T>,
...
class __tree {
 ...
 public:
  ...
  void _copy(node_pointer *__src, node_pointer __dst,
             node_pointer __parent = nullptr);
  ...
  node_pointer _find_lca(node_pointer __root, node_pointer __lhs,
                         node_pointer __rhs);

  node_pointer _insert_node(node_pointer __n, const_reference __ref,
                            value_compare comp_ = value_compare());
  void _display_node(node_pointer node, const std::string &prefix = "",
                     bool isLeft = true);
  ...
  node_pointer _create_node(const_reference __ref,
                            node_pointer __parent = nullptr);

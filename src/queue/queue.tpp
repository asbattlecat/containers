...
template <typename _T, class _Container>
queue<_T, _Container>::queue(std::initializer_list<value_type> const &__il)
    : c_(__il) {}
...
template <typename _T, class _Container>
queue<_T, _Container>::queue(queue<_T, _Container> &&__q)
    : c_(std::move(__q.c_)) {}
...
void queue<_T, _Container>::insert_many_back(Args &&...args) {
  static_assert((std::is_convertible<Args, _T>::value && ...),
                "queue insert many back must be convertible args");
